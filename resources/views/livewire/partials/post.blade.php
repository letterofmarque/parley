...
<div class="flex flex-col gap-3 {{ $indentClass }}" wire:key="parley-post-{{ $post->id }}">
    <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
        @if ($isDeleted)
            <x-id::text class="text-sm text-zinc-400 italic">{{ __('[deleted]') }}</x-id::text>
        @else
            <div class="flex items-center justify-between gap-2">
                <x-id::text class="text-sm font-medium">
                    {{ $post->user->name }}
            ...
            @if ($editingPost === $post->id)
                <form wire:submit="saveEdit" class="mt-2 flex flex-col gap-2">
                    <x-id::field name="editingBody">
                        <x-id::textarea wire:model="editingBody" rows="3" />
                    </x-id::field>

                    <div class="flex items-center gap-2">
                        <x-id::button type="submit" variant="primary" size="sm">{{ __('Save') }}</x-id::button>
                        <x-id::button variant="ghost" size="sm" wire:click="cancelEdit">{{ __('Cancel') }}</x-id::button>
                    </div>
                </form>
            @else
        ...
        <div class="{{ $depth < $maxDepth ? 'ml-6' : '' }}">
            <form wire:submit="submit" class="flex flex-col gap-2">
                <x-id::field name="body">
                    <x-id::textarea wire:model="body" rows="2" placeholder="{{ __('Write a reply…') }}" autofocus />
                </x-id::field>

                <div class="flex items-center gap-2">
                    <x-id::button type="submit" variant="primary" size="sm">{{ __('Reply') }}</x-id::button>
                    <x-id::button variant="ghost" size="sm" wire:click="cancelReply">{{ __('Cancel') }}</x-id::button>
                </div>
            </form>
        </div>
    @endif

<div class="flex h-full w-full flex-1 flex-col gap-4">
    <x-deck::heading size="xl">{{ __('Forum') }}</x-deck::heading>
                    ...
                    <tr wire:key="{{ $category->id }}">
                        <td class="px-3 py-2">
                            ...
                            @if ($category->description)
                                <x-deck::text class="mt-0.5 text-sm text-zinc-500">{{ $category->description }}</x-deck::text>
                            @endif
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="2" class="px-3 py-8 text-center">
                            <x-deck::text class="text-zinc-500">{{ __('No categories yet.') }}</x-deck::text>
                        </td>
                    </tr>
                @endforelse
            </tbody>

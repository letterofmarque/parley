<div class="flex h-full w-full flex-1 flex-col gap-4">
    <div class="flex items-center gap-4">
        <x-deck::button variant="ghost" :href="route('parley.forum.categories.index')" icon="arrow-left" wire:navigate>
            {{ __('Back') }}
        </x-deck::button>
    </div>

    <div class="flex items-center justify-between">
        <x-deck::heading size="xl">{{ $category->name }}</x-deck::heading>

        @can('create', \Marque\Parley\Models\Thread::class)
            <x-deck::button variant="primary" :href="route('parley.forum.threads.create', $category)" icon="plus" wire:navigate>
                {{ __('New Thread') }}
            </x-deck::button>
        @endcan
    </div>
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-3 py-8 text-center">
                            <x-deck::text class="text-zinc-500">{{ __('No threads yet.') }}</x-deck::text>
                        </td>
                    </tr>
                @endforelse
            </tbody>
